...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, coarse_ckpt_path=None):
    ...
    def gather_training_rays():
        ...
        if cfg_train.ray_sampler == 'in_maskcache':
            rgb_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz, radii_tr = dvgo.get_training_rays_in_maskcache_sampling(
            ...
            rgb_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz, radii_tr = dvgo.get_training_rays_flatten(
                rgb_tr_ori=rgb_tr_ori,
                train_poses=poses[i_train],
                HW=HW[i_train], Ks=Ks[i_train], ndc=cfg.data.ndc, inverse_y=cfg.data.inverse_y,
                flip_x=cfg.data.flip_x, flip_y=cfg.data.flip_y)
        else:
            rgb_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz, radii_tr = dvgo.get_training_rays(
                rgb_tr=rgb_tr_ori,
                train_poses=poses[i_train],
                HW=HW[i_train], Ks=Ks[i_train], ndc=cfg.data.ndc, inverse_y=cfg.data.inverse_y,
                flip_x=cfg.data.flip_x, flip_y=cfg.data.flip_y)
        ...
        return rgb_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz, batch_index_sampler, radii_tr

    rgb_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz, batch_index_sampler, radii_tr = gather_training_rays()
    ...
    for global_step in trange(1+start, 1+cfg_train.N_iters):
        ...
        if cfg_train.ray_sampler in ['flatten', 'in_maskcache']:
            ...
            radii = radii_tr[sel_b, sel_r, sel_c]
        else:
        ...
        # volume rendering
        render_result = model(rays_o, rays_d, viewdirs, radii, global_step=global_step, **render_kwargs)
